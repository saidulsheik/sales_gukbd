...
class Stores extends Admin_Controller 
{
    ...
    */
	public function create()
	{
		...
		$response = array();
		$this->form_validation->set_rules('store_code', 'Store Code', 'trim|required|is_unique[stores.store_code]');
		$this->form_validation->set_rules('store_name', 'Store Name', 'trim|required|is_unique[stores.store_name]');
		$this->form_validation->set_rules('store_contact', 'Store Contact No', 'trim|required');
		$this->form_validation->set_rules('store_email', 'Store Email Address', 'required|is_unique[stores.store_email]');
    ...
    */
	public function update($id)
	{
		...
		if($id) {
			...
			if(trim($this->input->post('edit_store_code')) != trim($this->input->post('old_store_code'))){
				$this->form_validation->set_rules('edit_store_code', 'Store Code', 'trim|required|is_unique[stores.store_code]');
			} else {
				$this->form_validation->set_rules('edit_store_code', 'Store Code', 'trim|required');
			}
		
			if(trim($this->input->post('edit_store_name')) != trim($this->input->post('old_store_name'))){
				$this->form_validation->set_rules('edit_store_name', 'Store Name', 'trim|required|is_unique[stores.store_name]');
			} else {
				$this->form_validation->set_rules('edit_store_name', 'Store Name', 'trim|required');
			}
			
			$this->form_validation->set_rules('edit_store_contact', 'Store Contact No', 'trim|required');
			$this->form_validation->set_rules('edit_store_email', 'Store Email Address', 'trim|required');
			$this->form_validation->set_rules('edit_store_address', 'Store  Address', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
	        		'store_code' => $this->input->post('edit_store_code'),
					'store_name' => $this->input->post('edit_store_name'),
					'store_contact' => $this->input->post('edit_store_contact'),
					'store_email' => $this->input->post('edit_store_email'),
					'store_address' => $this->input->post('edit_store_address'),
